...
class TransferDrug extends Contract {
  ...
  async createPO(ctx, buyerCRN, sellerCRN, drugName, quantity) {
    try {
      ...
      if (
        ...
        return {
          error:
            'Please make sure that the transfer of drug takes place in a hierarchical manner and no organisation in the middle is skipped. ',
        }
      }
    } catch (err) {
      return {
  ...
  async createShipment(ctx, buyerCRN, drugName, listOfAssets, transporterCRN) {
    ...
    try {
      ...
      let poIDBuffer = await ctx.stub
        .getState(poIDKey.value.key)
        .catch((err) => {
          console.log(err)
        })
      ...
      if (listOfAssetArray.length == poIDDetails.quantity) {
        ...
        return {
          error:
            "Either the drug Quantity doesn't match with PO, or drug ID is not valid",
        }
      }
      ...
      return {
        error:
          'Unable to create Shipment on the network, check input parameters',
        errorTrace: err.toString(),
      }
    }
  }
  ...
  async updateShipment(ctx, buyerCRN, drugName, transporterCRN) {
    ...
    try {
      ...
      return {
        error:
          'Unable to update Shipment on the network, check input parameters',
        errorTrace: err.toString(),
      }
    }
  }
  ...
  async retailDrug(ctx, drugName, serialNo, retailerCRN, customerAadhar) {
    try {
      ...
      if (drugDetail.owner != companyKey.value.key) {
        return {
          error:
            "This retailer doesn't own the Drug you are trying to buy, Please enter valid RetailerCRN",
        }
      }
